...
def main():
    ...
    for museum in get_museums():
        ...
        if 'lon_degree' in museum:
            ...
            lon = float(museum['lon_degree']) + float(museum['lon_degree']) / 60 + \
                float(museum['lon_degree']) / 3600
            lat = float(museum['lat_degree']) + float(museum['lat_degree']) / 60 + \
                float(museum['lat_degree']) / 3600
        else:
        ...
        features.append({
            'type': 'Feature',
            'geometry': {'type': 'point', 'coodinates': [lon, lat]}
        ...
        with open('museums.geohjson', 'w') as f:
...
def get_museum():
    ...
    # クエリの実行結果を反復処理する
    for result in response['result']['bindings']:
...
# yahoo!ジオコーダAPUのURL
YAHOO_GEOCODER_API_URL = 'http://geo.search.olp.yahooapis.jp/OpenLocalPlatform/V1/getCoder'
# DBM(ファイルを使ったキーバリュー型のDB)をジオコーディング結果のキャッシュとして
# 使用する。この変数はdictと同じように扱える。
geocoding = dbm.open('geocoding.db', 'c')
...
def geocode(address):
    ...
    if address not in getcoding_cache:
        ...
        # キーや値にはbyte型しか使えないが、str型は自動的にbyte型に変換される
        geocoding_cache[address] = response_text

    # キャッシュ内のAPIレスポンスをdictに変換
    # 値はbyte型なので、文字列として扱うにはデコードが必要
    reaponse = json.loads(geocoding_cache[address].decode('utf-8'))
    ...
    coodinates = response['Feature'][0]['Geometry']['Coordinates'].split(',')

    # floatのペアに変換して返す
    return (float(coodinates[0]), float(coodinates[1]))
