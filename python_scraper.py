...
def fetch(url):
	...
	webページのエンコーディングはContext-Typeヘッダから取得する。
	戻り値：str型のhtml
	'''
	f = urlopen(url)
...
def scrape(html):
	...
	books = []
	for partial_html in re.findall(r'<a itempop="url".*?</ul>\s*</a></li>', html, re.DOTALL):
		# 書籍のurlはitemprop="url"という属性を持つa要素のhref属性から取得する
		url = re.search(r'<a itemprop="url" href="(.*?)">', partial_html).group(1) 
		url = 'http://sample.scraping-book.com' + url # /で始まっているのでドメイン名などを追加する
...
def save(db_path, books):
	...
	戻り値：なし
	'''
	conn = sqlite3.connect(db_path) # データベースを開き、コネクションを取得
	...
	# execute()メソッドでは、複数のパラメータをリストで指定できる
