...
def find_textboxes_recursively(layout_obj):
	...
	# LTTextBoxを継承するオブジェクトの場合は、1要素のリストを返す
	if isinstance(layout_obj, LTTextBox):
		return [layout_obj]

	# LTConteinerを継承するオブジェクトは子要素を含むので、再帰的に探す
...
laparams = LAParams(detect_vertical=True) # Layout Analysticsのパラメータを設定。 縦書きの検出を有効にする
resource_manager = PDFResourceManager() # 共有のリソースを管理するリソースマネージャを作成
...
with open(sys.argv[1], 'rb') as f: # ファイルをバイナリ形式で開く
	...
	# 時間がかかるファイルは、キーワード引数pagenosで処理するページ番号(0はじまり)のリストを指定すると良い
